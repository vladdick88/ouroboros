...
def init_state() -> Dict[str, Any]:
    ...
    try:
        ...
        # Moonshot does not provide ground truth API - use local tracking only
        ground_truth = check_moonshot_ground_truth()
        if ground_truth is not None:
            st["session_total_snapshot"] = ground_truth["total_usd"]
            st["moonshot_total_usd"] = ground_truth["total_usd"]
            st["moonshot_daily_usd"] = ground_truth["daily_usd"]
            st["moonshot_last_check_at"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
        else:
...
def check_moonshot_ground_truth() -> Optional[Dict[str, float]]:
    """
    Moonshot AI does not provide a usage endpoint like OpenRouter.
    Returns None - budget tracking relies on local calculation.
    """
    # Moonshot API does not have a /auth/key endpoint for usage data
    # Budget tracking is done locally via token counting and pricing
    return None
...
def update_budget_from_usage(usage: Dict[str, Any]) -> None:
    ...
    # Step 2: Moonshot does not provide ground truth API - skip external budget check
        ...
        if ground_truth is not None:
            ...
            try:
                ...
                if session_total_snap is not None and session_spent_snap is not None:
                    ...
                    ms_delta = current_total_usd - _to_float(session_total_snap)
                    our_delta = current_spent_usd - _to_float(session_spent_snap)

                    if ms_delta > 0.001:
                        drift_pct = abs(ms_delta - our_delta) / max(abs(ms_delta), 0.01) * 100.0
                        st["budget_drift_pct"] = drift_pct
                        abs_diff = abs(ms_delta - our_delta)
